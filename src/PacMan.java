...
public class PacMan {
	...
	protected int x_direction;			// 0 : not moving
	// - 1 : left
	// 1 : right
	...
	public PacMan(int x_coordinate, int y_coordinate){
	...
	public Rectangle getBounds(){
		return new Rectangle(x_coordinate, y_coordinate, 
	...
	public void move(Component c){
		...
		if (!isDead && 
				// left
				(x_direction == -2 && x_coordinate > 0) ||
				// right
				(x_direction == 2 && x_coordinate + imageResource.getImage().getIconWidth() < c.getWidth())) {
			x_coordinate += x_direction;
		}
		if (!isDead &&
				// up
				(y_direction == -2 && y_coordinate > 0) ||
				// down
				(y_direction == 2 && y_coordinate + imageResource.getImage().getIconHeight() < c.getHeight())) {
			y_coordinate += y_direction;
		}	
		// jump
		...
		imageResource.updateImage(x_direction + y_direction, isDead);
	}
	...
	public void die() {
		isDead = true;
		imageResource.resetImageCounter();
	}
	...
	public void draw(Graphics g, Component c) {
		...
		//System.out.println(imageResource.getImage().getIconWidth());
		// left
		if(x_direction < 0)
			g2.drawImage(imageResource.getImage().getImage(), x_coordinate + imageResource.getImage().getIconWidth() + 9, 
					y_coordinate + 9, -imageResource.getImage().getIconWidth(), imageResource.getImage().getIconHeight(), null);
		//right
		else if(x_direction > 0)
			g2.drawImage(imageResource.getImage().getImage(), x_coordinate + 9, 
					y_coordinate + 9, imageResource.getImage().getIconWidth(), imageResource.getImage().getIconHeight(), null);
		// up
		else if(y_direction < 0) {
			g2.rotate(270 * (Math.PI/180), x_coordinate + imageResource.getImage().getIconWidth() / 2, y_coordinate + imageResource.getImage().getIconHeight() / 2);
			g2.drawImage(imageResource.getImage().getImage(), x_coordinate + 9, 
					y_coordinate + 9, imageResource.getImage().getIconWidth(), imageResource.getImage().getIconHeight(), null);
			g2.rotate(-270 * (Math.PI/180), x_coordinate + imageResource.getImage().getIconWidth() / 2, y_coordinate + imageResource.getImage().getIconHeight() / 2);
			// down
		} else if(y_direction > 0) {
			g2.rotate(90 * (Math.PI/180), x_coordinate + imageResource.getImage().getIconWidth() / 2, y_coordinate + imageResource.getImage().getIconHeight() / 2);
			g2.drawImage(imageResource.getImage().getImage(), x_coordinate + 9, 
					y_coordinate - 9, imageResource.getImage().getIconWidth(), imageResource.getImage().getIconHeight(), null);
			g2.rotate(-90 * (Math.PI/180), x_coordinate + imageResource.getImage().getIconWidth() / 2, y_coordinate + imageResource.getImage().getIconHeight() / 2);
		}
